...
gulp.task("sass-dev", function () {
    return gulp.src(srcSassFiles).pipe(sass(config.sass).on("error", sass.logError)).pipe(gulp.dest(distStyleDir));
});
...
gulp.task("copy", function () {
    return getFoldersSrc("html", copy).pipe(changed(distMainDir)).pipe(gulp.dest(distMainDir));
});

// Copy folders defined in copy array
gulp.task("copy-pug", function () {
    return getFoldersSrc("pug", copy).pipe(changed(distMainDir)).pipe(gulp.dest(distMainDir));
});
...
gulp.task("default", gulp.series(setDev, gulp.parallel("pug", "vendor", "sass-dev", "copy-pug"), serve, watchPug));

// Dev Gulp Task - HTML
// 1. Process HTML, vendor dir, SCSS and copy static assets
// 2. Init Browser Sync
// 3. Watch SCSS files, HTML files and static assets
gulp.task("dev-html", gulp.series(setDev, gulp.parallel("html", "vendor", "sass-dev", "copy"), serve, watch));

// Build Gulp Task - Pug
// 1. Process HTML, vendor dir, SCSS and copy static assets
// 2. Init Browser Sync
// 3. Watch SCSS files, HTML files and static assets
gulp.task("build", gulp.series(setBuild, "clean", gulp.parallel("vendor", "pug", "sass-build", "copy-pug")));

// Build Gulp Task - HTML
// 1. Clean dist folder
// 2. Process HTML, vendor dir, SCSS w/ source maps and minification and copy static assets
gulp.task("build-html", gulp.series(setBuild, "clean", gulp.parallel("vendor", "html", "sass-build", "copy")));
